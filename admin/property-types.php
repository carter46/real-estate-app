...
<div class="admin-panel">
  <h2><?= $editing ? 'Edit type' : 'Add type' ?></h2>
...
<div class="admin-panel" style="margin-top:1.5rem;">
  <h2>All types</h2>
  <?php if ($rows === []): ?>
    <p class="admin-note">No property types yet. Add one above.</p>
  <?php else: ?>
        ...
        <tr>
          ...
          <td>
            ...
            <?php if (!empty($row['is_active'])): ?>
              <form method="post" style="display:inline" onsubmit="return confirm('Deactivate this type?');">
                <?= csrf_field() ?>
