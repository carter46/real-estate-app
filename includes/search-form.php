...
?>
<form method="get" action="<?= e($action) ?>" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
  <div>
    <label class="font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant block mb-2" for="location">Location</label>
  ...
  <div>
    <label class="font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant block mb-2" for="price">Price Range</label>
    <select id="price" name="price" class="w-full bg-transparent border-0 border-b border-on-surface focus:ring-0 focus:border-primary font-body-md text-body-md px-0 py-2">
      ...
      <option value="1-5" <?= $price === '1-5' ? 'selected' : '' ?>>$1M–$5M</option>
      <option value="5-10" <?= $price === '5-10' ? 'selected' : '' ?>>$5M–$10M</option>
      <option value="10+" <?= $price === '10+' ? 'selected' : '' ?>>$10M+</option>
    </select>
  </div>
  <button type="submit" class="h-12 w-12 md:justify-self-end bg-primary text-on-primary flex items-center justify-center hover:bg-primary-container transition-colors" aria-label="Search">
